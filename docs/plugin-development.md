...
**而且它的粒度是秒。**`WithDelay` 收 `time.Duration`，线上传的是整秒，所以不足一秒的部分**向上取整**：`WithDelay(600*time.Millisecond)` 实际是 1 秒。这里向上而不是向下是有意的 —— 契约是「不早于」，向下截断会破坏它（这个函数原先就是截断的：600ms 变成 0，等于没有延迟；1.5s 变成 1s，比要求的早半秒可投递，且都不报错）。要亚秒精度的调度，队列不是那个工具。
