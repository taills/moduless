...
每个 Host RPC 挂在哪个权限上，由 `TestEveryRPCIsGatedOnItsOwnPermission` 逐条核对 —— 30 个，含三个不需要权限的（`GetConfig`、`Log`、`RecordMetric`；拒绝它们会让作者连「为什么被拒」都看不到）。它**两侧都验**：授予「除 X 之外的全部」必须被拒，只授予 X 必须不被拒。后者防的是**过度设限**，那让作者去申请一个审核者随后无法解释的权限 —— 和漏设门一样是缺陷，而只测前者会放过它。
...
| | 写法 | 适合 | 代价 |
|---|---|---|---|
| `digest` | 结构体字段持有小接口 | 新插件；一个插件有多个依赖 | 要有个结构体来放 |
| `apikey` | 包级 `var resolveKey = lookup` | 已经围绕包级状态写成的代码；只有一两个依赖 | 全局可变状态：测试要还原，且不能并发替换 |

新插件优先用前者。后者是给已经那样写成的代码的轻量选项，`apikey` 里注明了这个取舍。

**三个可以直接抄的样板**：`ratelimit`（纯 filter 判定）、`redact`（短路替换、后端 header 的保留）、`apikey`（认证与授权两个阶段、fail_closed 的错误处理），以及 `digest`（接口接缝、定时任务、出站与文件）。
...
| 场景 | 每请求 | 分配 | 约合 |
|---|---|---|---|
| Core 自己的路由（对照） | 41 µs | 86 | 24,500 req/s |
| 插件路由，无 filter | 106 µs | 306 | 9,400 req/s |
| ＋ 1 个 filter | 158 µs | 435 | 6,300 req/s |
| ＋ 3 个 filter | 263 µs | 686 | 3,800 req/s |
| ＋ 5 个 filter | 355 µs | 937 | 2,800 req/s |

每多一个命中的 filter 大约 **+50 µs、+126 次分配**，其中约 37 µs 是跨进程往返本身。这个量级决定了一件事：**filter 要按路径精确订阅，而不是靠代码里 `if` 掉不关心的请求** —— 后者已经付过了跨进程的钱。

> **绝对值会漂，比值不会。**这张表重测过一次：Core 路由那行从 52 µs 变成 41 µs，而同一天在旧提交上测也是 41 µs —— 差异全在 Go 版本和机器状态，不在代码。所以拿三个月前的绝对值去对今天的机器没有意义，**能跨环境用的是关系**：一个插件请求约是 Core 内部路由的 **2.6 倍**（106 vs 41），其中跨进程那一跳在整条请求里占 **65 µs**，约合 1.6 个 Core 路由；每多一个 filter 再加约半个插件请求（50 vs 106）。做容量规划和写 `match.paths` 用的正是这些比值。
>
> 还有一点：这些数字全部来自 `go test -bench`，负载是 `tests/fixtures/echoplugin`。那个 fixture 也是量具的一部分 —— 它的默认响应长了两个 header，就足以让这张表整体偏移 10%（实测：55 次多出的分配里 50 次来自它）。`TestTheBenchmarkFixtureStaysLean` 现在钉住了它的形状。
>
> [`docs/deployment.md`](deployment.md) 里那张延迟表**不是同一把尺子** —— 那是真实整栈、每档 300 次请求、带数据库的 p50/p99，含并发排队和真实 HTTP 服务端。两张表的数字不能互换着读。
