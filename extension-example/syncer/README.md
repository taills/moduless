...
**队列满的时候，重排这条路会断。** 重新发布也是一次入队，而 Core 在插件积压达到上限后会拒绝入队 —— 于是「安全路径」恰好在系统有压力时变回不安全的：实测上限设成 3，6 条里 **4 条进死信**。

这时正确的动作不是重排，而是**等锁**：消息本来就在队列里，往一个已经超限的积压里再塞一条只会更糟，而阻塞在这里既不花重试预算也不占积压 —— 这正是满队列想要的背压。加上这层回退之后，同样的场景是 `{Done:6 Dead:0}`，**一次重排都没有发生**，比无上限时的 21 次还少。
